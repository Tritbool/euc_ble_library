...
| `NinebotProtocol` | ◑ WheelLog + legacy frame support + serial/firmware carry-forward + Z settings readback carry-forward | ✅ startup + periodic query plan (`REQUEST_*`) | ◑ explicit support matrix + query commands | ◑ | Z BMS/auth response decoding and fuller settings coverage |
| `NinebotZProtocol` | ◑ decode delegated to `NinebotProtocol` with Z settings snapshot exposure | ✅ dedicated startup handshake + periodic keepalive/realtime/BMS polling | ◑ explicit support matrix incl. speed/alarm/calibrate/custom | ◑ | Z BMS/auth response decoding, individual param polling refinement, settings roundtrip validation |
