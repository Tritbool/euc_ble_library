...
class InMotionV2ProtocolTest {
    ...
    @Test
    fun testCanHandle() {
        // Test InMotion V2 device by name pattern
        val inMotionV2Device = MockBLEUtils.createMockDevice(
            name = "InMotion V11",
            manufacturerId = 0x0000
        )
        assertTrue(protocol.canHandle(inMotionV2Device))

        // Test InMotion V2 device by V12 pattern
        ...
        // Test non-InMotion V2 device
        val nonInMotionV2 = MockBLEUtils.createMockDevice(
            name = "KingSong S18",
            manufacturerId = 0x004E
        )
        assertFalse(protocol.canHandle(nonInMotionV2))

        // Test InMotion V8 (should not be handled by V2 protocol)
        val v8Device = MockBLEUtils.createMockDevice(
            name = "InMotion V8",
            manufacturerId = 0x0000
        )
        assertFalse(protocol.canHandle(v8Device))
    }

    @Test
    fun testDecodeValidPacket() {
        // Create a valid InMotion V2 packet with checksum
        // Start marker: 0x55
    ...
    fun testManufacturerAndSupportedModels() {
        ...
        assertTrue(protocol.supportedModels.contains("V11"))
        assertTrue(protocol.supportedModels.contains("V12"))
