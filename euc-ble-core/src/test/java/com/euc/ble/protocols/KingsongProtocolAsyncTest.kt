...
class KingsongProtocolAsyncTest {
    ...
    fun testSingleCompleteFrameEmitsEucData() = runBlocking {
        ...
        val collector = async(start = CoroutineStart.UNDISPATCHED) { collectN(protocol.dataFlow, 1, 1000L) }
        protocol.decode(payload)

        val items = collector.await()
    ...
    fun testTwoFramesInOnePayload() = runBlocking {
        ...
        val collector = async(start = CoroutineStart.UNDISPATCHED) { collectN(protocol.dataFlow, 2, 2000L) }
        protocol.decode(payload)
    ...
    fun testTruncatedThenCompletedFrame() = runBlocking {
        ...
        val collector = async(start = CoroutineStart.UNDISPATCHED) { collectN(protocol.dataFlow, 1, 1500L) }
