...
/**
 * Unit tests for NinebotZProtocol - tests the Ninebot Z EUC protocol implementation
 */
class NinebotZProtocolTest {

    private val protocol = NinebotZProtocol()
    ...
    @Test
    fun testCanHandle() {
        // Test Ninebot Z device by name pattern
        val ninebotZDevice = MockBLEUtils.createMockDevice(
            name = "Ninebot Z10",
            manufacturerId = 0x004E
        )
        assertTrue(protocol.canHandle(ninebotZDevice))

        // Test Ninebot Z device by Z pattern
        val zDevice = MockBLEUtils.createMockDevice(
            name = "Ninebot Z6",
            manufacturerId = 0x0000
        )
        assertTrue(protocol.canHandle(zDevice))

        // Test non-Ninebot Z device
        val nonNinebotZ = MockBLEUtils.createMockDevice(
            name = "KingSong S18",
            manufacturerId = 0x004E
        )
        assertFalse(protocol.canHandle(nonNinebotZ))

        // Test Ninebot S2 (should not be handled by Z protocol)
        val s2Device = MockBLEUtils.createMockDevice(
            name = "Ninebot S2",
            manufacturerId = 0x0000
        )
        assertFalse(protocol.canHandle(s2Device))
    }

    @Test
    fun testDecodeValidPacket() {
        // Create a valid Ninebot Z packet with checksum and motor temperature
        // Start marker: 0x55
        // Frame length: 0x1C (28 bytes)
        ...
        // Motor temperature: 0x1E 0x00 = 30 (3.0°C)
        // Checksum: calculated
        val data = byteArrayOf(
            0x55.toByte(), // Start marker
            0x1C.toByte(), // Frame length (28 bytes)
            ...
            0x00.toByte(), // Additional data
            0x00.toByte(), // Additional data
            0x1E.toByte(), 0x00.toByte(), // Motor temperature (3.0°C)
            0x00.toByte(), // Additional data
    ...
    fun testDecodeInvalidPacket() {
        ...
        val shortData = byteArrayOf(0x55.toByte(), 0x1C.toByte(), 0x01.toByte())
    ...
    fun testDecodeEdgeCases() {
        ...
        val zeroData = byteArrayOf(
            ...
            0x00.toByte(), 0x00.toByte(), 0x00.toByte(), 0x00.toByte(),
            0x00.toByte(), 0x00.toByte(), // Motor temperature (0.0°C)
            0x00.toByte(),
        ...
        val maxData = byteArrayOf(
            ...
            0x00.toByte(), 0x00.toByte(), 0x00.toByte(), 0x00.toByte(),
            0xFF.toByte(), 0xFF.toByte(), // Motor temperature (6553.5°C)
            0x00.toByte(),
            0x00.toByte() // Checksum (will be calculated)
        )
    ...
    fun testCreateCommand() {
        ...
        assertArrayEquals(byteArrayOf(0x55.toByte(), 0x06.toByte(), 0x01, 0x01, 0x01, 0x00, 0x00, 0x00), lightOn)

        // Test light off command
        val lightOff = protocol.createCommand(CommandType.LIGHT_OFF, 0)
        assertArrayEquals(byteArrayOf(0x55.toByte(), 0x06.toByte(), 0x01, 0x01, 0x00, 0x00, 0x00, 0x00), lightOff)

        // Test beep command
        val beep = protocol.createCommand(CommandType.BEEP, 0)
        assertArrayEquals(byteArrayOf(0x55.toByte(), 0x05.toByte(), 0x02, 0x01, 0x00, 0x00, 0x00), beep)

        // Test power off command
        val powerOff = protocol.createCommand(CommandType.POWER_OFF, 0)
        assertArrayEquals(byteArrayOf(0x55.toByte(), 0x05.toByte(), 0x03, 0x01, 0x00, 0x00, 0x00), powerOff)

        // Test light brightness command
        val brightness50 = protocol.createCommand(CommandType.LIGHT_BRIGHTNESS, 50)
        assertArrayEquals(byteArrayOf(0x55.toByte(), 0x06.toByte(), 0x04, 0x7F.toByte(), 0x00, 0x00, 0x00, 0x00), brightness50)

        val brightness100 = protocol.createCommand(CommandType.LIGHT_BRIGHTNESS, 100)
        assertArrayEquals(byteArrayOf(0x55.toByte(), 0x06.toByte(), 0x04, 0xFF.toByte(), 0x00, 0x00, 0x00, 0x00), brightness100)

        val brightness0 = protocol.createCommand(CommandType.LIGHT_BRIGHTNESS, 0)
        assertArrayEquals(byteArrayOf(0x55.toByte(), 0x06.toByte(), 0x04, 0x00.toByte(), 0x00, 0x00, 0x00, 0x00), brightness0)
    ...
    fun testManufacturerAndSupportedModels() {
        ...
        assertTrue(protocol.supportedModels.contains("Ninebot Z"))
        assertTrue(protocol.supportedModels.contains("Ninebot Z10"))
        assertTrue(protocol.supportedModels.contains("Ninebot Z6"))
    }
